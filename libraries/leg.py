...
#---#-------- KONSTRUKTOR -------------------------------------------#
    ...
    def __init__(self, leg):
        ...
        self.a = [self.legDict["phys"][0],self.legDict["phys"][1],self.legDict["phys"][2],self.legDict["phys"][3],self.legDict["phys"][4],self.legDict["phys"][5],self.legDict["phys"][6]]
...
#---#-------- BEWEGUNG ----------------------------------------------#
    ...
    def setLegMovement(self):
        ...
        """
        print("Bein:",self.legDict["index"])
        print("Winkel:",alpha,beta,gamma)
        print("Geschw:",alphaSpeed,betaSpeed,gammaSpeed)
        print()
        """
        #-- Anweisung
        try:
...
#---#-------- BERECHNUNGEN ------------------------------------------#
    ...
    def angleSpeed(self):
        ...
        #print("oldServoAngles:",oldServoAngles)
        ...
        for i in range(0,3):
            ...
            """
            print("oldServoAngles[i]:",oldServoAngles[i])
            print("self.legDict[servos][i][angle]", self.legDict["servos"][i]["angle"])
            print("distances value:",distances[i+1]["ref"]["index"])
            print("distances rate:",distances[i+1]["ref"]["angle"])
            """
        ...
        #print('Leg: ',self.legDict["index"],' -- Servo: ',self.legDict["servos"][0]["index"],' -- Speed: ',self.legDict["servos"][0]["speed"])
        #print('Leg: ',self.legDict["index"],' -- Servo: ',self.legDict["servos"][1]["index"],' -- Speed: ',self.legDict["servos"][1]["speed"])
        #print('Leg: ',self.legDict["index"],' -- Servo: ',self.legDict["servos"][2]["index"],' -- Speed: ',self.legDict["servos"][2]["speed"])
        #print()
...
#---#-------- SERVO-INITIALISIERUNG ---------------------------------#
    ...
    #----------------------------------------------------------------#
    def servoInit(self):
        self.servo1 = JointDrive(id=self.legDict["servos"][0]["index"], ccw=self.legDict["servos"][0]["ccw"], aOffset=self.legDict["servos"][0]["offset"])
        self.servo2 = JointDrive(id=self.legDict["servos"][1]["index"], ccw=self.legDict["servos"][1]["ccw"], aOffset=self.legDict["servos"][1]["offset"])
        self.servo3 = JointDrive(id=self.legDict["servos"][2]["index"], ccw=self.legDict["servos"][2]["ccw"], aOffset=self.legDict["servos"][2]["offset"])